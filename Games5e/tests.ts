...
function showCards1(area?: Area) {
  ...
  table.ctx.clearRect(area.left, area.top,
...
function SSendGame() {
  ...
  if (selGame.codeName != "SandS") { return }
  ...
  let randomPiles: number[] = [];
  let botSuit = 0;
  let mySelGame = selGame as SevenAndSix;
  pack.makeShuffledArray(randomPiles, 13);
  for (let i = 0; i < 7; i++) {
    let pile = table.piles[randomPiles[i]];
    if (choices.suits != 1) { botSuit = 1 - botSuit } // switch between Spades, Hearts for King bottom card
  ...
  function AddCards(pile: Pile, suit: number, from: number, to: number) {
    let y = pile.y;
    let x = pile.x;
    for (let card52I = from; card52I >= to; card52I--) {
      y += table.yStep;
      pile.addCardP(card52I + suit * 13, x, y);
      if (choices.suits != 1) { suit = 1 - suit } // switch between Spades, Hearts
    }
...
function SSendGame2() {
  ...
  if (selGame.codeName != "SandS") { return }
  if (choices.suits != 1) { suit = 0 }
  AddCards(table.piles[0], 11, 4);
  AddCards(table.piles[1], 3, 0);
  table.showCards();

  function AddCards(pile: Pile, from: number, to: number) {
    let game = selGame as SevenAndSix;
...
function URendGame() {
  ...
  for (let card52I = 0; card52I < 7 * 13; card52I++) {
    ...
    if (card52I % 13 == 12) {y += table.cardHeight / 4 }
  }
...
function test_rotate() {
  ...
  table.ctx.strokeRect(card.area.left, card.area.top,
    (card.area.right - card.area.left), (card.area.bottom - card.area.top));
...
function debuggingRotator() {
  ...
  function rotateX(x: number, y: number): number {
    return (x * Math.cos(radians) - y * Math.sin(radians))
  }
  function rotateY(x: number, y: number): number {
...
function areaRound(a: Area) {
  a.top = Math.round(a.top);
  a.left = Math.round(a.left);
  a.bottom = Math.round(a.bottom);
  a.right = Math.round(a.right);
}
function logAreas(a1: Area, a2: Area, a3: Area) {
  areaRound(a1); areaRound(a2); areaRound(a3);
...
function logCoords(where: string) {
...
function FIXinitialiseTable() {
  ...
  for (let shIx = 1; shIx <= 103; shIx++) {
    ...
    if (shIx % 8 == 0) { talonX += 4 }
  }
  // the acePile
  let AAgame = selGame as any; // to get at acePile
...
function doShuffle(numPacks: number) {
