...
class UserTester {
  ...
  gameElem = document.getElementById("selGame") as HTMLSelectElement;
  nameElem = document.getElementById("fname") as HTMLInputElement;
  updateDB: UpdateDB;
  ...
  action(theAction: string, col1?: number) {

    let message = this.nameElem.value + "/";
    if (typeof (col1) == 'undefined') { col1 = 0 }
    switch (theAction) {
      case "List":
      case "Time":
        message += theAction + "/";
  ...
  starts10() {
    ...
    let nameElem = document.getElementById("fname") as HTMLInputElement;
    for (let i = 0; i <= 9; i++) {
