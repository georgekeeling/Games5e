...
class SeniorWrangler extends Game {
    ...
    requestDrop(pileI, cardI, x, y) {
        ...
        if (pileI <= 15) {
            pileI += 8;
        } // always on lowPiles where we would move to
        if (dragPile.cards[0].rank() != this.calcRankNeeded(pileI - 8)) {
            return false;
        }
    ...
    nextDealTargets(target) {
        ...
        if (table.piles[SWtalonI].cards.length == 0) {
            return null;
        }
        target.faceUp = true;
        target.angle = 0;
        ...
        function returnLongPile(candiPile) {
    ...
    hintEllipse() {
        ...
        for (let pileI = 16; pileI < 24; pileI++) {
            if (table.piles[pileI - 8].cards.length > 1) {
                continue;
            }
            let ranksNeeded = [];
            ...
            let pile = table.piles[pileI];
            if (pile.cards.length > 0) {
                lowCardRank = pile.endCard().rank();
            }
            ranksNeeded[ranksNeeded.length] = this.calcRankNeeded2(keyCardRank, lowCardRank);
            while (ranksNeeded[ranksNeeded.length - 1] != 13) {
                ranksNeeded[ranksNeeded.length] = this.calcRankNeeded2(keyCardRank, ranksNeeded[ranksNeeded.length - 1]);
    ...
    hint() {
        ...
        for (let pileI of randomOrderUpperPiles) {
            ...
            for (let keyPileI of randomOrderKeyPiles) {
                keyPileI += 8;
                if (table.piles[keyPileI].cards.length > 1) {
                    continue;
                }
                if (this.calcRankNeeded(keyPileI) == pile.endCard().rank()) {
...
//# sourceMappingURL=seniorWrangler.js.map