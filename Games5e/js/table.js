...
class Area {
    ...
    overlapsRange(xMin1, xMax1, xMin2, xMax2) {
        return (xMax1 >= xMin2 && xMax2 >= xMin1);
    }
}
class Card {
    constructor(pile, cards52I, xPos, yPos, faceUp, angle) {
        this.pile = pile;
    ...
    jinkable(cardOnTop) {
        ...
        if (cardOnTop.suit() == 1 || cardOnTop.suit() == 2) {
            colourTopper = 1;
        }
        if (this.rank() == cardOnTop.rank() + 1) {
            if (colourTopper != colourThis || choices.suits == 1) {
                return false;
            }
        }
        return true;
...
class Pile {
    ...
    constructor(cardIx, x, y, faceUp, angle) {
        // unless pile contains rotated or jinked  cards
        this.cards = []; // cards in the pile. Obviously!
    ...
    addCardP(cardI, x, y, faceUp, angle) {
        let newI = this.cards.length;
        if (typeof (faceUp) == 'undefined') {
            faceUp = true;
        }
        ;
...
class Table {
    constructor(empty) {
        this.cardScale = 1;
        this.SVGwidth = 360; // 360 = width from SVG file.
        this.SVGheight = 540;
        this.width = 1000;
        this.height = 600;
        this.lockLevel = 0;
    ...
    lock() { this.lockLevel++; }
    unlock() { if (this.lockLevel > 0) {
        this.lockLevel--;
    } }
    isLocked() { return (this.lockLevel > 0); }
    resize() {
        if (table.isLocked()) {
            return;
        }
    ...
    welcome() {
        ...
        this.ctx.fillText("16.3.23", this.width / 2, baseY + spacing * 4);
        this.setCtxFontSize(this.siteWindow.welcomFont);
    ...
    showCards(area) {
        ...
        this.ctx.clearRect(area.left, area.top, (area.right - area.left), (area.bottom - area.top));
        for (let pileIx = 0; pileIx <= this.piles.length - 1; pileIx++) {
    ...
    addPile(cardIx, x, y, faceUp, angle) {
        this.piles[this.piles.length] = new Pile(cardIx, x, y, faceUp, angle);
    }
    addCardT(pile, cardIx, x, y, faceUp, angle) {
    ...
    dealN(undoReset, afterDeal) {
        // subsequent deals
        if (typeof (undoReset) == 'undefined') {
            undoReset = true;
        }
        if (typeof (afterDeal) == 'undefined') {
            afterDeal = this.doNothing;
        }
        this.dealX(this.doNothing, selGame.nextDealNTargets, undoReset, afterDeal);
    }
    dealX(initDeal, nextInDeal, undoReset, afterDeal) {
        ...
        function deal1() {
            if (pos >= moves) {
                ...
                if (table.piles[selGame.talonPileI].cards.length == 0) {
                    targetPile = null;
                } // safety if nextInDeal forgot.
                dragPile.emptyDrag();
                if (targetPile == null) {
                    clearInterval(id);
                    if (undoReset) {
    ...
    flyPile(targetPile, x, y, endFunction) {
    ...
    flyOutBack(sourcePile, sourceCardI, targetPile, yOffset, extract1Card) {
...
class Pack {
    constructor() {
        this.cards52 = []; // 52 card images
        this.cardBack = document.getElementById("cBack0");
        this.suitLetters = ['s', 'h', 'd', 'c'];
        this.shuffled = [];
        let j = 0;
    ...
    makeShuffledArray(array, length) {
