...
class AuntyAlice extends Game {
  emptyPileI = -1;      // one pile (<=23) might be empty.Must be filled
  ...
  override click(pileI: number, cardI: number) {
    ...
    if (pileI > this.talonPileI) { sound.soundFail(); return }
    ...
    for (let candidateI of checkPiles) {
      ...
      if (candiCard.rank() == rank - 3 && candiCard.suit() == suit) {
        if (rank > 7 && candiPile.cards.length == 1) { continue };
        let x = candiPile.x;
        let y = candiPile.y;
        if (candiPile.cards.length > 0) {y += 4 }
        undo.saveState();
        pile.spliceToDrag(cardI);
  ...
  override requestDrag(pileI: number, cardI: number): number {
    ...
    if (pileI < 24 && pile.cards.length == 1) { return Infinity } 
  ...
  override requestDrop(pileI: number, cardI: number, x?: number, y?: number): boolean {
    ...
    if (pileI == this.talonPileI) { return false }

    let pile = table.piles[pileI];

    if (this.emptyPileI != -1) {
      // must drop 2,3 or 4 on empty pile in correct row
      if (pileI != this.emptyPileI) { return false }
      if (card.rank() > 4) { return false }
      let row = Math.floor(pileI / 8) + 2;
      if (row != card.rank()) { return false }
    ...
    if (card.rank() == 1) {
      ...
      if (pileI != acePileI) { return false }
      dragPile.moveTo(pile.x, pile.y +
    ...
    if (card.rank() != targetCard.rank() + 3) { return false }

    let pileBaseRank = Math.floor(pileI / 8) + 2;   // 2,3 or 4
    let neededBaseRank = (card.rank() - 5) % 3 + 2;
    if (neededBaseRank != pileBaseRank) { return false }

    let neededCardsBelow = Math.floor((card.rank() - 5) / 3) + 1;
    if (neededCardsBelow != pile.cards.length) { return false }

    dragPile.moveTo(pile.x, pile.y + 4);
    finishUp(this);
    return true;

    function finishUp(myThis: AuntyAlice) {   // 'this' is undefined in here!
  ...
  override nextDealTargets(target: DealTarget): Pile {
    ...
    target.pileI++;
    if (target.pileI >= selGame.talonPileI) { return null }
    ...
    if (target.pileI < 24) {
      ...
      // bottom row
      if (dealCard.rank() >= 11) { target.angle = 45 }
    }
    return tPile;
  }
  ...
  override nextDealNTargets(target: DealTarget): Pile {
    ...
    target.pileI++;
    if (target.pileI == 32) {return null }
    let dealCard = table.piles[selGame.talonPileI].endCard()
    ...
    if (pile.cards.length > 0) {
      ...
      if (dealCard.rank() >= 11 && card.angle == 45) { target.angle = 45 }
    }
    else {
      target.x = pile.x;
      target.y = pile.y;
  ...
  override initialiseTable() {
    ...
    function AAbreaker(shIx: number): boolean {
  ...
  override hint() {
    ...
    let checkAces = (pile0: number, pileN: number): boolean => {
    ...
    function checkCard(pile0: number, pileN: number): boolean {
      let randomOrderPiles = [];
      pack.makeShuffledArray(randomOrderPiles, pileN - pile0);
      for (let pileI of randomOrderPiles) {
        pileI += pile0;
