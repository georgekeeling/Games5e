...
class UncleRemus extends Game {
  ...
  override requestDrag(pileI: number, cardI: number): number {
    ...
    for (let i = cardI + 1; i < pile.cards.length; i++) {
      if (card.suit() != pile.cards[i].suit()) { return 0 }
      let rankNeeded = card.rank() - i + cardI;
      if (rankNeeded != pile.cards[i].rank()) { return 0 }
    }
  ...
  override requestDrop(pileI: number, cardI: number, x?: number, y?: number): boolean {
    ...
    if (pileI < 0 || pileI > 9) { return false }
    let pile = table.piles[pileI];
    ...
    function finishUp(myThis: UncleRemus) {      // 'this' does not in work. could use => but I hate it
      pile.addDrag();
  ...
  private checkBookAndFly(pile: Pile, checkNext?: Function) {
  ...
  private checkNext() {
    let myThis = selGame as UncleRemus;   // as ever, this not working
    if (myThis.nextCheckPile > 10) { return }
    myThis.checkBookAndFly(table.piles[myThis.nextCheckPile++], myThis.checkNext);
  }

  private dealFinished() {
  ...
  override click(pileI: number, cardI: number) {
    ...
    if (otherPileI == -1) { otherPileI = this.checkOthers(pileI, bottomCard, false, false) }
    if (otherPileI == -1) { otherPileI = this.checkOthers(pileI, bottomCard, false, true) }
    if (otherPileI == -1) {
      sound.soundFail();
      return;
    }
  ...
  private checkOthers(thisPileI: number, bottomCard:Card, matchSuit: boolean, allowEmpty): number {
    ...
    for (let otherPileI of randomOrderPiles) {
      ...
      if (table.piles[otherPileI].endCard().rank() - 1 == bottomCard.rank()) {
        if (!matchSuit) { return otherPileI }
  ...
  override checkWin() {
    ...
    if (table.piles[URdiscardPile].cards.length == 104) { table.youWin() }
  ...
  override nextDealTargets(target: DealTarget): Pile {
    ...
    if (cardsDealt >= choices.dealCardsUR) { return null }
    target.pileI++;
  ...
  override initialiseTable() {
    ...
    function URbreaks(shIx: number): boolean {
  ...
  override hint() {
    ...
    for (let pileI of randomOrderPiles) {
      ...
      if (cardI == -1) { cardI = 0 }    // case when run is only thing in column 
