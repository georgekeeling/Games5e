...
class Kings extends Game{
  ...
  override requestDrop(pileI: number, cardI: number, x?: number, y?: number): boolean {
    ...
    if (0 <= pileI && pileI <= 7) {
      if (pile.cards.length == 13) { return false }   // we are done with that pile (in a duh way)
      ...
      if (requiredRank != dragCard.rank()) { return false }
      if (requiredSuit != dragCard.suit()) { return false }
      dragPile.moveTo(topCard.x, topCard.y);
      pile.addDrag();
      pile.endCard().setAngle(0);
      if (pile.cards.length == 13) { sound.soundSmallTrumpet() }   // we are done with that pile (in a good way)
    ...
    if (pileI == KiFanPileI) {
      if (this.dragSourcePile != KiFanPileI) { return false }
    ...
    function sortByX(cardA: Card, cardB: Card): number {
  ...
  override checkWin() {
    ...
    for (let pileI = 0; pileI <= 7; pileI++) {
      if (table.piles[pileI].cards.length < 13) { return }
    }
    table.youWin();
  }
  ...
  override nextDealNTargets(target: DealTarget): Pile {
    ...
    let myThis = selGame as Kings;

    target.pileI++;
    target.angle = 0;
    if (target.pileI == 0) { target.pileI = 8 }
    if (target.pileI == 8) {
      tidyFaceDownPile();
    }
    ...
    if (target.pileI == KiFanPileI) { target.pileI = KiFaceDownPileI }
  ...
  override initialiseTable() {
    ...
    for (let pileI = 0; pileI < 8; pileI++) {
      ...
      table.addPile(kingsFound[(pileI * 2) % 8], x,
        table.yStep + (table.cardHeight + table.yStep) * rowI,
        true, 0);
    }
    ...
    // Fan pile
    table.addPile(-1, leftKingsX,
      table.yStep + (table.cardHeight + table.yStep) * 4.5);  // y as above with rowI=4.5
    // face down pile
    table.addPile(-1, rightKingsX,
      table.yStep + (table.cardHeight + table.yStep) * 4.5);  // y as above
  ...
  override hint() {
    ...
    function searchKingPiles(suit: number, rank: number): Pile {
      let kingPiles = [];
      ...
      for (let pileI of kingPiles) {
        ...
        if (requiredRank == rank) { return pile }
      }
      return null;
    }
  }
