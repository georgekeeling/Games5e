...
class SevenAndSix extends Game {
  jinkOffset: number;
  nextCheckPile = -1;     // used when checking all piles for book-ism after deal
  ...
  override requestDrag(pileI: number, cardI: number): number {
    ...
    if (cardI == 0 && pile.cards.length == 13) { return 0 }
    return Infinity;
  }
  ...
  override requestDrop(pileI: number, cardI: number, x?: number, y?: number): boolean {
    ...
    if (pileI < 0 || pileI > 12) { return false }
    let pile = table.piles[pileI];
    ...
    function finishUp(myThis: SevenAndSix) { 
  ...
  override click(pileI: number, bottomCardI: number) {
    ...
    let topCardI: number;
    if (!bottomCard.faceUp) { sound.soundFail(); return; }
    if (this.isFinishedBook(pile)) { sound.soundFail(); return; }


    for (topCardI = bottomCardI + 1; topCardI < pile.cards.length; topCardI++) {
      if (pile.cards[topCardI].x != bottomCard.x) { break; }
    }
    topCardI--;
    // topCardI is now top card in sequence. May also be top card on pile
    if (un_collapse()) { return };
  ...
  private alignAngles(pile: Pile, coveredcardI: number) {
  ...
  private checkOthers(thisPileI: number, bottomCard: Card): number {
    ...
    if (KingAtBottom && bottomCard === table.piles[thisPileI].cards[0]) { return -1 }
    let randomOrderPiles = [];
    pack.makeShuffledArray(randomOrderPiles, 13);
    for (let otherPileI of randomOrderPiles) {
      if (otherPileI == thisPileI) { continue }
      let otherPile = table.piles[otherPileI];
      if (this.isFinishedBook(otherPile)) { continue }
      ...
      if (!otherPile.endCard().jinkable(bottomCard)) { return otherPileI }
    }
    return -1;
  }
  ...
  override checkWin() {
    ...
    for (let pileI = 0; pileI < 13; pileI++) {
      let pile = table.piles[pileI];
      if (pile.cards.length == 0) { emptyPiles++ }
      if (this.isFinishedBook(pile)) { finishedBooks++ }
    }
  ...
  override nextDealTargets(target: DealTarget): Pile {
    ...
    let myThis = selGame as SevenAndSix;
    ...
    if (target.pileI > 12) {
      ...
      if (tPile.cards.length * 13 >= choices.dealCards76) { return null }
    }
    ...
    switch (choices.dealCards76) {
      ...
      case 52:
        ...
        if (tPile.cards.length == 3) {
          ...
          if (pileCard.jinkable(dealCard)) { target.x += myThis.jinkOffset }
        }
        break;
      default:
  ...
  override nextDealNTargets(target: DealTarget): Pile {
    ...
    if (target.pileI > 12 || table.piles[13].cards.length == 0) { return null }
    target.angle = 0;
    target.faceUp = true;
    let pile = table.piles[target.pileI];
    target.x = pile.x;
    ...
    if (pileCard.jinkable(dealCard)) {
      ...
      for (let cardI = 0; cardI < pile.cards.length - 1; cardI++) {
        ...
        if (!lowerCard.faceUp) { continue }
        if (!lowerCard.jinkable(upperCard)) { continue }
  ...
  private isFinishedBook(pile: Pile): boolean {
  ...
  override initialiseTable() {
    ...
    function SandSbreaks(shIx: number): boolean {
  ...
  override hint() {
    ...
    for (let pileI of randomOrderPiles) {
      ...
      while (--cardI >= 0) {
        let card = pile.cards[cardI];
        if (card.x != runX || !card.faceUp) { break }
      }
      cardI++;
